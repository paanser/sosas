...
modo_precio = st.radio("Seleccionar modo de precio por m²:", ["Tarifa", "Manual"])
precio_m2 = 0.0
if modo_precio == "Tarifa":
    categoria = st.selectbox("Selecciona la categoría", list(categorias.keys()))
    producto = st.selectbox("Selecciona el producto", list(categorias[categoria].keys()))
    precio_m2 = categorias[categoria][producto]
else:
    producto = "Precio manual"
    precio_m2 = st.number_input("Introduce el precio por m²", min_value=0.0, step=0.1)
categoria = st.selectbox("Selecciona la categoría", list(categorias.keys()))
producto = st.selectbox("Selecciona el producto", list(categorias[categoria].keys()))
precio_m2 = categorias[categoria][producto]

# Cantos y lados (capturados fuera del botón para mantener valores)
...
# Incremento porcentual (también fuera)
...
if st.button("Calcular presupuesto"):
    ...
    st.write(f"Área original: {area_original:.2f} m²")
    ...
    st.markdown(f'**Total con IVA (21%) incluido: {total_con_iva:.2f} €**')
