...
# Elección de modo de precio
modo_precio = st.radio("¿Cómo quieres ingresar el precio por m²?", ["Seleccionar desde tarifa", "Ingresar manualmente"])


st.header("Dimensiones del cristal")
ancho_cm = st.number_input("Introduce el ancho (cm)", min_value=0.0, step=0.1)
alto_cm = st.number_input("Introduce el alto (cm)", min_value=0.0, step=0.1)

area_real = round((ancho_cm / 100) * (alto_cm / 100), 2)

# Buscar múltiplo superior
multiplo_alto = buscar_multiplo_superior(alto_cm / 100)
multiplo_ancho = buscar_multiplo_superior(ancho_cm / 100)
area_ajustada = round(multiplo_alto * multiplo_ancho, 2)

st.markdown(f"**Área real:** {area_real} m²")
st.markdown(f"**Área ajustada a múltiplos:** {area_ajustada} m²")
...
if modo_precio == "Ingresar manualmente":
    tipo_seleccionado = st.selectbox("Referencia de tarifa (solo informativa)", list(precios.keys()))
    precio_referencia = precios[tipo_seleccionado]
    st.markdown(f"**Precio de referencia:** ${precio_referencia:,.2f} por m²")
    precio_m2 = st.number_input("Introduce el precio por m² manualmente", min_value=0.0, value=precio_referencia)
else:
    categoria = st.selectbox("Selecciona una categoría", list(categorias.keys()))
    opciones = [f"{nombre} - ${precio:,.2f}" for nombre, precio in categorias[categoria]]
    seleccion = st.selectbox("Selecciona el tipo de cristal", opciones)
    tipo_seleccionado = seleccion.split(" - $")[0]
    precio_m2 = precios[tipo_seleccionado]
